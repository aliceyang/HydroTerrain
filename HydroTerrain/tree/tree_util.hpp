...
template <class T>
void print_tree_tabbed(const tree<T> &tr, std::ostream& str);

template <class T>
void print_subtree_tabbed(const tree<T>& tr, typename tree<T>::iterator iRoot, std::ostream& str);


// Alice DONE: Print the whole tree (all the roots) in a tabbed structure
template <class T>
void print_tree_tabbed(const tree<T> &tr, std::ostream& str)
{
	...
	for(typename tree<T>::sibling_iterator iRoots = tr.begin(); iRoots != tr.end(); ++iRoots, ++headNum) {
		print_subtree_tabbed(tr,iRoots,str);

	}
}

// Alice DONE: Print this node and everything under it in a  tabbed tree structure
template <class T>
void print_subtree_tabbed(const tree<T>& tr, typename tree<T>::iterator iRoot, std::ostream& str)
{
	typename tree<T>::iterator it = iRoot;
	typename tree<T>::iterator end = tr.end(iRoot);
	while (it != end)
	{
